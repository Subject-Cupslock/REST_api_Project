...
def test_get_items_only_own(client: TestClient, db_session):
    headers_a = register_and_login(client, email='userA@example.com', username='userA', password='passA')
    headers_b = register_and_login(client, email='userB@example.com', username='userB', password='passB')
...
def test_get_item_owner_only(client: TestClient, db_session):
    headers_a = register_and_login(client, email='owner@example.com', username='owner', password='pass')
    headers_b = register_and_login(client, email='intruder@example.com', username='intruder', password='pass')
...
def test_update_item_success(client: TestClient, db_session):
    headers = register_and_login(client, email='upd@example.com', username='upduser', password='pass')
...
def test_update_item_not_owner(client: TestClient, db_session):
    headers_a = register_and_login(client, email='own@example.com', username='own', password='pass')
    headers_b = register_and_login(client, email='notown@example.com', username='notown', password='pass')

    r_create = client.post('/items/', json={'title': 'Mine'}, headers=headers_a)
    item_id = r_create.json()['id']

    r_upd = client.put(f'/items/{item_id}', json={'title': 'Hack', 'description': 'try'}, headers=headers_b)
...
def test_delete_item_success(client: TestClient, db_session):
    headers = register_and_login(client, email='del@example.com', username='deluser', password='pass')
...
def test_delete_item_not_owner(client: TestClient, db_session):
    headers_a = register_and_login(client, email='owner2@example.com', username='owner2', password='pass')
    headers_b = register_and_login(client, email='stranger2@example.com', username='stranger2', password='pass')
